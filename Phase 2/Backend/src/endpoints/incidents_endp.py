...
from src.database.staff_db import update_staff_availability
from src.database.incident_db import (create_incident, fetch_all_incidents, fetch_incidents_by_id, fetch_incidents_by_station, delete_incident_by_id, update_incident_station_name, update_incident_status, assign_to_staff, fetch_incidents_by_userid)
...
@router.post("/assign_incident_to_staff")
def assign_incident_to_staff(id: str = Form(...), staff_id: str = Form(...)):
    if id == "" or staff_id == "":
        return {"ERROR": "MISSING PARAMETERS"}
    assign_to_staff(id, staff_id)
