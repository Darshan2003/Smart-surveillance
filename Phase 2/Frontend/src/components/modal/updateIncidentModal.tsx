...
const UpdateIncidentModal = ({
  ...
  const assignToStaff = async (id, staff_id) => {
    let data = new FormData();
    data.append("id", id);
    data.append("staff_id", staff_id)
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/incidents/assign_incident_to_staff`, 
         data
      );
      if(res.status==200)
      {
        ...
        <ModalContent className="!z-[1002] !m-auto !w-max min-w-[340px] !max-w-[85%] top-[3vh] md:top-[5vh]">
          ...
          <ModalBody className=" overflow-x-hidden">
           ...
           hover:cursor-pointer hover:bg-gray-100 dark:bg-navy-700 dark:text-white dark:hover:bg-white/20 dark:active:bg-white/10 p-3`}
                ...
                {incident?.status !== "Closed" && (
                  <div className="mt-3">
                    <Select label="Reassign Incident" onChange={(e)=> assignToStaff(incident.id, e)} >
                      {availableStaff.map((staff) => <Option key={staff.id} value={staff.id}>{staff.staff_name}</Option>)}
                    </Select>
                  </div>
                )}
              </div>
